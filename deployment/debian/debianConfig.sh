...
sudo apt install bash binutils bison coreutils diffutils findutils gawk gcc grep gzip audacity gimp vlc htop vim gnome-boxes gparted pavucontrol neofetch cmus xmonad xmobar libghc-xmonad-extras-dev ghc yt-dlp links brasero sound-juicer picard git xfonts-terminus fonts-terminus nala qjackctl geary make virt-manager gnome-firmware fwupdate vcdimager arch-install-scripts synaptic software-properties-gtk lshw unattended-upgrades vrms distrobox podman keepassxc dconf-editor gnome-console devede inkscape gnome-tweaks obs-studio v4l2loopback-dkms
